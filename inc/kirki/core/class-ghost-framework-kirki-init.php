<?php
/**
 * Initializes Ghost_Framework_Kirki
 *
...
/**
 * Initialize Ghost_Framework_Kirki
 */
class Ghost_Framework_Kirki_Init {
    ...
    public function __construct() {
        ...
        new Ghost_Framework_Kirki_Values();
        new Ghost_Framework_Kirki_Sections();
    }

    /**
     * Properly set the Ghost_Framework_Kirki URL for assets.
     *
     * @static
     * @access public
     */
    public static function set_url() {
        if ( Ghost_Framework_Kirki_Util::is_plugin() ) {
            return;
        }
    ...
    /**
     * Add the default Ghost_Framework_Kirki control types.
     *
     * @access public
     * @since 3.0.0
    ...
    public function default_control_types( $control_types = array() ) {
        $this->control_types = array(
            'checkbox'              => 'Ghost_Framework_Kirki_Control_Checkbox',
            'kirki-color'           => 'Ghost_Framework_Kirki_Control_Color',
            'kirki-custom'          => 'Ghost_Framework_Kirki_Control_Custom',
            ...
            'upload'                => 'Ghost_Framework_Kirki_Control_Upload',
        );
    ...
    public function register_control_types() {
        ...
        $skip_control_types = apply_filters(
            ...
            array(
                'Ghost_Framework_Kirki_Control_Repeater',
    ...
    public function add_panels() {
        if ( ! empty( Ghost_Framework_Kirki::$panels ) ) {
            foreach ( Ghost_Framework_Kirki::$panels as $panel_args ) {
                // Extra checks for nested panels.
                if ( isset( $panel_args['panel'] ) ) {
                    if ( isset( Ghost_Framework_Kirki::$panels[ $panel_args['panel'] ] ) ) {
                        // Set the type to nested.
                        $panel_args['type'] = 'kirki-nested';
                    }
                }

                new Ghost_Framework_Kirki_Panel( $panel_args );
            }
        }
    }
    ...
    public function add_sections() {
        if ( ! empty( Ghost_Framework_Kirki::$sections ) ) {
            foreach ( Ghost_Framework_Kirki::$sections as $section_args ) {
                // Extra checks for nested sections.
                if ( isset( $section_args['section'] ) ) {
                    if ( isset( Ghost_Framework_Kirki::$sections[ $section_args['section'] ] ) ) {
                        // Set the type to nested.
                        $section_args['type'] = 'kirki-nested';
                        // We need to check if the parent section is nested inside a panel.
                        $parent_section = Ghost_Framework_Kirki::$sections[ $section_args['section'] ];
                        if ( isset( $parent_section['panel'] ) ) {
                            $section_args['panel'] = $parent_section['panel'];
                        }
                    }
                }
                new Ghost_Framework_Kirki_Section( $section_args );
            }
        }
    }
    ...
    public function add_fields() {
        global $wp_customize;
        foreach ( Ghost_Framework_Kirki::$fields as $args ) {

            // Create the settings.
            new Ghost_Framework_Kirki_Settings( $args );
            ...
            if ( $wp_customize ) {
                // Create the control.
                new Ghost_Framework_Kirki_Control( $args );
    ...
    private function fields_from_filters() {
        ...
        if ( ! empty( $fields ) ) {
            foreach ( $fields as $field ) {
                Ghost_Framework_Kirki::add_field( 'global', $field );
            }
        }
    }

    /**
     * Alias for the is_plugin static method in the Ghost_Framework_Kirki_Util class.
    ...
    public static function is_plugin() {
        return Ghost_Framework_Kirki_Util::is_plugin();
    }

    /**
     * Alias for the get_variables static method in the Ghost_Framework_Kirki_Util class.
     * This is here for backwards-compatibility purposes.
     *
     * @static
     * @access public
    ...
    public static function get_variables() {
        ...
        _doing_it_wrong( __METHOD__, esc_html__( 'We detected you\'re using Ghost_Framework_Kirki_Init::get_variables(). Please use Ghost_Framework_Kirki_Util::get_variables() instead.', '@@text_domain' ), '3.0.10' );
        return Ghost_Framework_Kirki_Util::get_variables();
    }
    ...
    public function remove_panels( $wp_customize ) {
        foreach ( Ghost_Framework_Kirki::$panels_to_remove as $panel ) {
    ...
    public function remove_sections( $wp_customize ) {
        foreach ( Ghost_Framework_Kirki::$sections_to_remove as $section ) {
    ...
    public function remove_controls( $wp_customize ) {
        foreach ( Ghost_Framework_Kirki::$controls_to_remove as $control ) {
    ...
    public function admin_notices() {
        ...
        <div class="notice notice-info is-dismissible">
            <p>
                <?php esc_html_e( 'Your theme uses a Font Awesome field for icons. To avoid issues with missing icons on your frontend we recommend you install the official Font Awesome plugin.', '@@text_domain' ); ?>
            </p>
            <p>
                <a class="button button-primary" href="<?php echo esc_url( admin_url( 'plugin-install.php?tab=plugin-information&plugin=font-awesome&TB_iframe=true&width=600&height=550' ) ); ?>"><?php esc_html_e( 'Install Plugin', '@@text_domain' ); ?></a>
                <a class="button button-secondary" href="<?php echo esc_url( wp_nonce_url( admin_url( '?dismiss-nag=font-awesome-kirki' ), 'kirki-dismiss-nag', 'nonce' ) ); ?>"><?php esc_html_e( 'Don\'t show this again', '@@text_domain' ); ?></a>
            </p>
        </div>
        <?php
    }
