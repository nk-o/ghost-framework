<?php
/**
 * The Ghost_Framework_Kirki API class.
 ...
 */
class Ghost_Framework_Kirki extends Ghost_Framework_Kirki_Init {

    /**
     * Absolute path to the Ghost_Framework_Kirki folder.
     *
     * @static
     * @access public
     * @var string
    ...
    /**
     * URL to the Ghost_Framework_Kirki folder.
     *
     * @static
     * @access public
     * @var string
    ...
    public static function get_option( $config_id = '', $field_id = '' ) {
        return Ghost_Framework_Kirki_Values::get_value( $config_id, $field_id );
    }
    ...
    public static function add_config( $config_id, $args = array() ) {
        $config                             = Ghost_Framework_Kirki_Config::get_instance( $config_id, $args );
    ...
    public static function add_field( $config_id, $args ) {
        if ( doing_action( 'customize_register' ) ) {
            _doing_it_wrong( __METHOD__, esc_html__( 'Ghost_Framework_Kirki fields should not be added on customize_register. Please add them directly, or on init.', '@@text_domain' ), '3.0.10' );
        }
        ...
        $classname = 'Ghost_Framework_Kirki_Field_' . str_replace( ' ', '_', ucwords( $str ) );
        if ( class_exists( $classname ) ) {
            new $classname( $config_id, $args );
            return;
        }
        if ( false !== strpos( $classname, 'Ghost_Framework_Kirki_Field_Kirki_' ) ) {
            $classname = str_replace( 'Ghost_Framework_Kirki_Field_Kirki_', 'Ghost_Framework_Kirki_Field_', $classname );
            if ( class_exists( $classname ) ) {
                new $classname( $config_id, $args );
                return;
            }
        }
        new Ghost_Framework_Kirki_Field( $config_id, $args );
    }
