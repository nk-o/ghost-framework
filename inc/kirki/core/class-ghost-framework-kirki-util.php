<?php
/**
 * A utility class for Ghost_Framework_Kirki.
 *
 ...
 * Utility class.
 */
class Ghost_Framework_Kirki_Util {
    ...
    /**
     * Determine if Ghost_Framework_Kirki is installed as a plugin.
     *
     * @static
     * @access public
     * @since 3.0.0
    ...
    public static function is_plugin() {
        ...
        foreach ( $plugins as $plugin => $args ) {
            if ( ! $is_plugin && isset( $args['Name'] ) && ( 'Ghost_Framework_Kirki' === $args['Name'] || 'Ghost_Framework_Kirki Toolkit' === $args['Name'] ) ) {
                $is_plugin = true;
                $_plugin   = $plugin;
            }
        }

        // No need to proceed any further if Ghost_Framework_Kirki wasn't found in the list of plugins.
        if ( ! $is_plugin ) {
            return false;
        }
    ...
    public static function get_variables() {
        ...
        foreach ( Ghost_Framework_Kirki::$fields as $field ) {
            ...
            if ( isset( $field['variables'] ) && $field['variables'] && ! empty( $field['variables'] ) ) {
                ...
                foreach ( $field['variables'] as $field_variable ) {
                    ...
                    if ( isset( $field_variable['name'] ) ) {
                        ...
                        $variables[ $field_variable['name'] ] = Ghost_Framework_Kirki_Values::get_value( $field['settings'] );
                        if ( $variable_callback ) {
                            $variables[ $field_variable['name'] ] = call_user_func( $field_variable['callback'], Ghost_Framework_Kirki_Values::get_value( $field['settings'] ) );
                        }
                    }
                }
            }
    ...
    public function http_request( $request = array(), $url = '' ) {
        ...
        foreach ( $plugins['plugins'] as $plugin ) {
            if ( isset( $plugin['Name'] ) && 'Ghost_Framework_Kirki Toolkit' === $plugin['Name'] ) {
                $exists = true;
            }
        }
        // Inject data.
