<?php
/**
 * The Ghost_Framework_Kirki autoloader.
 ...
 * @since 3.0.10
 */
class Ghost_Framework_Kirki_Autoload {
    ...
    /**
     * The Ghost_Framework_Kirki class autoloader.
    ...
    protected function autoload( $class_name ) {
        ...
        // Not a Ghost_Framework_Kirki file, early exit.
        if ( 0 !== stripos( $class_name, 'Ghost_Framework_Kirki' ) ) {
            return;
        }
    ...
    protected function get_paths( $class_name ) {
        ...
        $name_parts = explode( '_', str_replace( 'Ghost_Framework_Kirki_', '', $class_name ) );

        // Handle modules loading.
        if ( isset( $name_parts[0] ) && 'Modules' === $name_parts[0] ) {
            $path    = dirname( __FILE__ ) . '/modules/';
            $path   .= strtolower( str_replace( '_', '-', str_replace( 'Ghost_Framework_Kirki_Modules_', '', $class_name ) ) ) . '/';
            $paths[] = $path . $filename;
        }
        ...
        $substr   = str_replace( 'Ghost_Framework_Kirki_', '', $class_name );
