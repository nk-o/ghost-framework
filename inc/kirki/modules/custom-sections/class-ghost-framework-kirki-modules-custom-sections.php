 ...
 */
class Ghost_Framework_Kirki_Modules_Custom_Sections {
    ...
    public function set_section_types( $section_types ) {
        $new_types = array(
            'kirki-default'  => 'Ghost_Framework_Kirki_Sections_Default_Section',
            'kirki-expanded' => 'Ghost_Framework_Kirki_Sections_Expanded_Section',
            'kirki-nested'   => 'Ghost_Framework_Kirki_Sections_Nested_Section',
            'kirki-link'     => 'Ghost_Framework_Kirki_Sections_Link_Section',
        );
    ...
    public function set_panel_types( $panel_types ) {
        $new_types = array(
            'kirki-nested' => 'Ghost_Framework_Kirki_Panels_Nested_Panel',
        );
    ...
    public function include_sections_and_panels() {
        ...
        foreach ( $section_types as $id => $class ) {
            if ( ! class_exists( $class ) ) {
                $path = wp_normalize_path( $folder_path . 'class-ghost-framework-kirki-sections-' . $id . '-section.php' );
                if ( file_exists( $path ) ) {
                    include_once $path; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude
                    continue;
                }
                $path = str_replace( 'class-ghost-framework-kirki-sections-kirki-', 'class-ghost-framework-kirki-sections-', $path );
                if ( file_exists( $path ) ) {
                    include_once $path; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude
                }
            }
        ...
        foreach ( $panel_types as $id => $class ) {
            if ( ! class_exists( $class ) ) {
                $path = wp_normalize_path( $folder_path . 'class-ghost-framework-kirki-panels-' . $id . '-panel.php' );
                if ( file_exists( $path ) ) {
                    include_once $path; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude
                    continue;
                }
                $path = str_replace( 'class-ghost-framework-kirki-panels-kirki-', 'class-ghost-framework-kirki-panels-', $path );
                if ( file_exists( $path ) ) {
                    include_once $path; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude
                }
            }
    ...
    public function enqueue_scrips() {
        wp_enqueue_style( 'kirki-custom-sections', trailingslashit( Ghost_Framework_Kirki::$url ) . 'modules/custom-sections/sections.min.css', array(), '@@theme_version' );
        wp_enqueue_script( 'kirki-custom-sections', trailingslashit( Ghost_Framework_Kirki::$url ) . 'modules/custom-sections/sections.min.js', array( 'jquery', 'customize-base', 'customize-controls' ), '@@theme_version', false );
    }
}
