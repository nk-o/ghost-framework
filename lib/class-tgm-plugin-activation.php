 ...
 * @version   2.6.1 for parent theme @@theme_name for publication on WordPress.org
			...
			if ( ! empty( $desired_slug ) ) {
				...
				if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
					...
					if ( true === $GLOBALS['wp_filesystem']->move( $from_path, $to_path ) ) {
						...
						return new WP_Error(
							'rename_failed',
							esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', '@@text_domain' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', '@@text_domain' ),
							array(
								'found'    => $subdir_name,
					...
					return new WP_Error(
						'packaged_wrong',
						esc_html__( 'The remote plugin package consists of more than one file, but the files are not packaged in a folder.', '@@text_domain' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', '@@text_domain' ),
						array(
							'found'    => $subdir_name,
							'expected' => $desired_slug,
						)
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function no_items() {
			echo esc_html__( 'No plugins to install, update or activate.', '@@text_domain' ) . ' <a href="' . esc_url( self_admin_url() ) . '"> ' . esc_html( $this->tgmpa->strings['dashboard'] ) . '</a>';
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
					...
					public function add_strings() {
						if ( 'update' === $this->options['install_type'] ) {
							...
							if ( version_compare( $this->tgmpa->wp_version, '4.8', '<' ) ) {
								$this->upgrader->strings['skin_update_successful'] .= ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', '@@text_domain' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', '@@text_domain' ) . '</span>.</a>';
							}
						}
					}
